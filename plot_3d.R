...
pca.tg <- getPcaUmapMetaData(S.O.T4)




fig <- plot_ly(pca.tg, x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, color = ~customclassif)
...
Idents(S.O.T2) <- 'customclassif'
FeaturePlot(S.O.T2, features = c("Pdgfra", 'Pdgfrb'), reduction = 'umap', label = T)
