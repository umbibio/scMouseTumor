...
## This function is taken from: https://github.com/IanevskiAleksandr/sc-type
## The source is modified to accept only positive gene set
## Citation: https://www.nature.com/articles/s41467-022-28803-w
sctype_score <- function(expr.mat, gs){
  
  # check input matrix
  if(!is.matrix(expr.mat)){
    warning("expr.mat doesn't seem to be a matrix")
  } else {
    if(sum(dim(expr.mat))==0){
      warning("The dimension of input expr.mat matrix equals to 0, is it an empty matrix?")
    }
  }
  ...
  gs = lapply(1:length(gs), function(d_){ 
    GeneIndToKeep = rownames(expr.mat) %in% as.character(gs[[d_]]); rownames(expr.mat)[GeneIndToKeep]})
...
addCellId <- function(S.O, markers.list){
  
  es.max <- sctype_score(S.O[["RNA"]]@scale.data,  markers.list) 
  # merge by cluster
  cL_resutls <- do.call("rbind", lapply(unique(S.O@meta.data$seurat_clusters), function(cl){
    es.max.cl = sort(rowSums(es.max[ ,rownames(S.O@meta.data[S.O@meta.data$seurat_clusters==cl, ])]), 
                     decreasing = !0)
    head(data.frame(cluster = cl, type = names(es.max.cl), 
                    scores = es.max.cl, ncells = sum(S.O@meta.data$seurat_clusters==cl)), 10)
  }))
  ...
  S.O@meta.data$customclassif = ""
  for(j in unique(sctype_scores$cluster)){
    cl_type = sctype_scores[sctype_scores$cluster==j,]; 
    S.O@meta.data$customclassif[S.O@meta.data$seurat_clusters == j] = as.character(cl_type$type[1])
  }
