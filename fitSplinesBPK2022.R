...
xx <- readRDS('../../data/input/fromJosh/BPK2022_tumor/midpoint_tumor2022_lfc.rds')
yy <- readRDS('../../data/input/fromJosh/BPK2022_tumor/midpoint_tumor2022_pseudotime.rds')
cell.idents <- readRDS('../../data/input/fromJosh/BPK2022_tumor/midpoint_tumor2022_ident.rds')

yy <- data.frame(cells = names(yy), pt = c(yy))
yy$cellType <- cell.idents

xx <- as.data.frame(as.matrix(xx))
xx$GeneName <- rownames(xx)
#xx.long$cells <- gsub('2022_', '', xx.long$cells)
...
saveRDS(trans.time, '../../data/input/rds/trans_time_bpk2022.rds')
